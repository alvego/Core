...
---Помечаем труп как уже обысканный/ободранный
---@param unitGUID string unitGUID (for ignore in FindCorpse)
---@param skining? boolean Default = false (`true` подходит для снятия шкур, иначе для обыска)
function c.bLootedCorpse(unitGUID, skining)
    if not bConnected then return nil end
    return cmd('LootedCorpse', unitGUID, skining)
end
